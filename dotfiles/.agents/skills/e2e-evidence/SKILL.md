...
- Store agent-generated evidence under `e2e/evidences/` because this repository convention is local-only and gitignored.
- Use a scoped subfolder such as `e2e/evidences/<change-or-session>/screenshots/` for intentional screenshots.
...
- Configure Playwright projects that need agent evidence with an artifact root under `e2e/evidences`, for example `outputDir: 'e2e/evidences/playwright'`.
...
- If validation uses browser automation outside a Playwright test file, still save screenshots under `e2e/evidences/<change-or-session>/screenshots/`.
