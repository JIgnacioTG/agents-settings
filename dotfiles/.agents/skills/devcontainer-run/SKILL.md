...
When both projects are selected, launch the two `bunx @devcontainers/cli up` commands concurrently as independent shell tasks. Do not run Reserhub first and wait before starting Adara unless the user explicitly requests sequential execution or one project depends on the other.
...
When both projects were started in parallel, sync MCP auth for each successful project in parallel after its corresponding `up` command completes.
