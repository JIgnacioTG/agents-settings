...
- Cursor grouped plans should include a per-group `project scope` and `branch suggestion`, and any grouped plan with parallel work should include a parallel execution trace that shows fan-out, whether each fan-in is merge-required or completion-only, and where serialization resumes.
- Cursor grouped plans must use `recommended agent` metadata, not Codex-style `execution profile` metadata.
- Cursor grouped plans must separate `implementation` groups from `test/coverage` groups, and every `test/coverage` group must declare `test scope` as `unit`, `integration`, or `e2e`.
- New Cursor grouped artifacts must route main execution through `@implementation-agent`.
- During grouped execution, delegate `implementation` groups directly to `@implementation-agent`.
- Before any Cursor `test/coverage` lane starts, run `@test-setup-explorer`, then pass its findings into the downstream `@implementation-agent` execution for that lane.
