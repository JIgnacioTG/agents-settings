...
- During grouped execution, use a scoped explore handoff only when the active group complexity is `unknown`; otherwise delegate implementation directly with the grouped plan and the execution-critical context already in hand.
- Cursor grouped plans should include a per-group `branch suggestion`, and any grouped plan with parallel work should include a parallel execution trace that shows fan-out, required merge groups, and where serialization resumes.
- When Cursor grouped work runs in parallel, prefer detached worktrees by default unless the user explicitly requests branch-per-group execution, and require the declared merge group before resuming downstream serialized work.
