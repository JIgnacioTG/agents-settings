...
- During grouped execution, use a scoped explore handoff only when the selected implementation route is Spark-capable, if supported by that tool, or the active group complexity is `unknown`; otherwise delegate implementation directly with the grouped plan and the execution-critical context already in hand.
