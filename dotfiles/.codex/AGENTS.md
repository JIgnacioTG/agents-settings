...
- During grouped execution, run a scoped explore delegate only when Spark is actually selected for the active group or the active group complexity is `unknown`; otherwise delegate implementation directly with the grouped plan and the execution-critical context already in hand.
