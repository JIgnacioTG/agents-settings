...
- When fast mode is not already active and the user has not requested it, Codex Spark is optional only for `medium` and `high` grouped work when the tradeoff is worth offering. If fast mode is unavailable or declined, continue with the declared non-fast execution profile.
