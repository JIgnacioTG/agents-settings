...
- Code review is opt-in only. Do not invoke `requesting-code-review`, review agents, or review workflows unless the user explicitly asks for review or the active plan/artifact explicitly requires review. Exceptions: plan-based execution, ultrawork mode, and OpenSpec flows always require a final code review over the changed scope using the relevant review workflow; during plan writing/generation (including openspec), code review is also required as the last task using relevant code review skills.
...
- When applying approved OpenSpec artifacts or executing an already-written plan as-is, do not invoke `test-driven-development` or `verification-before-completion`; this includes `/openspec-apply-change`, and `/opsx:apply` flows unless the user explicitly asks to rewrite the plan. The only exception is to use `verification-before-completion` when the active group is explicitly a verification group, which is usually the last group. Even in those execution flows, always finish with changed-scope formatter, linter, typecheck, related tests, and a code review. For OpenSpec flows, also run `openspec-verify-change` as the final OpenSpec-specific verification step.
