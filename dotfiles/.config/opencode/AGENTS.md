...
- OpenCode grouped plans should include a per-group `project scope` and `branch suggestion`, and any grouped plan with parallel work should include a parallel execution trace that shows fan-out, whether each fan-in is merge-required or completion-only, and where serialization resumes.
- OpenCode grouped plans must use `recommended agent` metadata, not Codex-style `execution profile` metadata.
- OpenCode grouped plans must separate `implementation` groups from `test/coverage` groups, and every `test/coverage` group must declare `test scope` as `unit`, `integration`, or `e2e`.
- New OpenCode grouped artifacts must route main execution through `@implementation-agent`. Do not emit `@implementation-agent-fast`, `@implementation-agent-medium`, `@implementation-agent-spark`, or `@implementation-agent-thinker` in new grouped plans.
- During grouped execution, delegate `implementation` groups directly to `@implementation-agent`.
- Before any OpenCode `test/coverage` lane starts, run `@test-setup-explorer`, then pass its findings into the downstream `@implementation-agent` execution for that lane.
- The `@test-setup-explorer` agent is exploration-only. It must analyze existing implementation, tests, fixtures, factories, seeds, helpers, and harness setup, then report the setup that the downstream implementation lane should reuse or create.
- When OpenCode grouped work runs in parallel, prefer detached worktrees by default unless the user explicitly requests branch-per-group execution. Require the declared merge group before resuming downstream serialized work only when same-`project scope` fan-in needs a shared merge; if the upstream groups are in different `project scope` values and no shared merge is needed, wait for completion only.
